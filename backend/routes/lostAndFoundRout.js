...
lostAndFoundRoute.get("/lost-item", (req, res) => {
  ...
  const query = ` SELECT lost_item.*, users.name FROM lost_item JOIN users ON lost_item.user_id = users.user_id`;
...
lostAndFoundRoute.get("/found-item", (req, res) => {
  //     //   const { username } = req.user;

  const query = ` SELECT found_item.*, users.name FROM found_item JOIN users ON found_item.user_id = users.user_id`;
  db.query(query, (err, results) => {
    if (err) {
